...
class IntentClassifier:
    ...
    def is_symptom_related_spacy(self, text: str) -> bool:
        ...
        medical_ents = {'DISEASE', 'SYMPTOM', 'TREATMENT'}
...
FALLBACK_SYMPTOMS = {
    "fever": {"systems": ["general"], "synonyms": ["feverish", "high temperature", "feeling hot", "temperature", "febrile"]},
    "cough": {"systems": ["respiratory"], "synonyms": ["coughing", "coughed", "hacking"]},
    "sore throat": {"systems": ["respiratory"], "synonyms": ["throat pain", "scratchy throat", "throat hurts", "painful throat"]},
    "runny nose": {"systems": ["respiratory"], "synonyms": ["stuffy nose", "nasal congestion", "congestion", "blocked nose", "stuffed up"]},
    "headache": {"systems": ["neuro"], "synonyms": ["head pain", "head ache", "migraine", "head hurts"]},
    "fatigue": {"systems": ["general"], "synonyms": ["tired", "exhausted", "wiped", "fatigued", "fatiqued", "tiredness", "exhaustion", "weary", "worn out"]},
    "chills": {"systems": ["general"], "synonyms": ["chilly", "shivering", "shaking"]},
    "nausea": {"systems": ["gi"], "synonyms": ["nauseous", "queasy", "sick to stomach", "feel sick"]},
    "vomiting": {"systems": ["gi"], "synonyms": ["vomit", "throwing up", "threw up", "vomited", "puking", "puked"]},
    "diarrhea": {"systems": ["gi"], "synonyms": ["diarrhoea", "loose stools", "runny stool", "watery stool"]},
    "abdominal pain": {"systems": ["gi"], "synonyms": ["stomach pain", "tummy ache", "cramps", "stomach ache", "belly pain", "stomach hurts"]},
    "shortness of breath": {"systems": ["respiratory"], "synonyms": ["breathless", "wheezing", "can't breathe", "difficulty breathing", "short of breath", "breathing difficulty"]},
    "chest pain": {"systems": ["cardio"], "synonyms": ["chest tightness", "chest hurts", "tight chest"]},
    "dizziness": {"systems": ["neuro"], "synonyms": ["lightheaded", "dizzy", "vertigo", "spinning"]},
    "rash": {"systems": ["derm"], "synonyms": ["skin rash", "bumps", "hives", "spots"]},
    "itching": {"systems": ["derm"], "synonyms": ["itchy", "itch", "scratchy"]},
    "body aches": {"systems": ["general"], "synonyms": ["aches", "soreness", "body ache", "muscle aches", "aching", "sore muscles"]},
    "sneezing": {"systems": ["respiratory"], "synonyms": ["sneeze", "sneezes", "sneezed"]},
    "loss of taste": {"systems": ["neuro"], "synonyms": ["ageusia", "can't taste", "no taste", "lost taste"]},
    "loss of smell": {"systems": ["neuro"], "synonyms": ["anosmia", "can't smell", "no smell", "lost smell"]},
}
...
class RuleBasedExtractor(SymptomExtractor):
    ...
    def __init__(self,
        ...
        # NEW: Add stemming variations for common suffixes
    ...
    def _add_variations(self):
        """Add common morphological variations to synonym map."""
        variations_to_add = {}
        
        for syn, canon in list(self.syn2canon.items()):
            base = syn.lower()
            
            # Add -ed, -ing, -s variations for verbs/nouns
            # e.g., "cough" -> "coughing", "coughed", "coughs"
            if not any(base.endswith(suffix) for suffix in ['ed', 'ing', 's', 'ness']):
                # Add -ed
                variations_to_add[base + 'ed'] = canon
                variations_to_add[base + 'd'] = canon  # e.g., fatigue -> fatigued
                
                # Add -ing
                variations_to_add[base + 'ing'] = canon
                
                # Add -s
                variations_to_add[base + 's'] = canon
                
                # Add -ness for adjectives
                if base.endswith('y'):
                    variations_to_add[base[:-1] + 'iness'] = canon  # dizzy -> dizziness
                else:
                    variations_to_add[base + 'ness'] = canon
            
            # Handle -e ending (e.g., ache -> aching, ached)
            if base.endswith('e') and len(base) > 3:
                variations_to_add[base[:-1] + 'ing'] = canon
                variations_to_add[base + 'd'] = canon
        
        # Add all variations to the map
        self.syn2canon.update(variations_to_add)
...
class RuleBasedChatbot:
    def __init__(self,
        ...
        self.intent_classifier = IntentClassifier() if use_intent_classification else None
    ...
    def process_user_input(self, text: str) -> str:
        ...
        if tl == "list":
            ...
            if pos:
                return f"So far I have collected the following symptoms: {self.format_symptom_list(pos)}."
        ...
        if self.intent_classifier:
            ...
            if intent == 'greeting':
                return (
                    "Hello! I'm here to help you check your symptoms. "
                    "What symptoms are you experiencing?"
                )
            elif intent == 'off_topic':
                return (
                    "I'm specifically designed to help assess medical symptoms. "
                    "Could you describe any physical symptoms you're experiencing? "
                    "For example: fever, cough, pain, nausea, etc."
                )
        ...
        while True:
            ...
            if user.lower() == "done":
                if len(self.state.positives()) >= self.state.k_threshold:
                    resp = self.process_confirmation("yes")
                    print("Bot:", resp)
